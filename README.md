...
모든 크기를 글자 기준값의 배수(rem)로 지정하고 기준값을 화면 폭에 연동했습니다. 어느 폰에서나 화면 폭 대비 글자 비율이 같고(344~430px 9개 기종 측정), 높이가 짧은 폰(아이폰 SE 등)은 여백을 줄여 홈 버튼이 한 화면에 들어옵니다. 두 손가락 확대는 막지 않습니다.
